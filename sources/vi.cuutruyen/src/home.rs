...
impl Home for CuuTruyen {
	fn get_home(&self) -> Result<HomeLayout> {
		...
		let responses: [core::result::Result<Response, RequestError>; 3] = Request::send_all([
			...
			// top week
			Request::get(format!(
				"{}/api/v2/mangas/top?duration=week&page=1&per_page=24",
				base_url
			))?,
			// top month
			Request::get(format!(
				"{}/api/v2/mangas/top?duration=month&page=1&per_page=24",
				base_url
			))?,
		])
		.try_into()
		...
		// spotlight mangas
		let manga_id = home?
			.get_json::<CuuSearchResponse<CuuHome>>()?
			.data
			.spotlight_mangas
			.iter()
		...
		let manga_res = Request::send_all(
			manga_id
				.iter()
				.map(|id| Request::get(format!("{}/api/v2/mangas/{}", base_url, id)).unwrap()),
		);
		...
		// latest mangas
		let latest_chapter = Request::get(format!("{}/api/v2/home_a", base_url))?
			.send()?
			.get_json::<CuuSearchResponse<CuuHome>>()?
			.data
			.new_chapter_mangas
