...
impl From<ComicItem> for Manga {
	fn from(item: ComicItem) -> Self {
		...
		let viewer = if item.categories.contains(&"WEBTOON".to_string()) {
			Viewer::Webtoon
		} else {
			Viewer::RightToLeft
		};
		...
		all_tags.extend(item.categories.iter().cloned());
...
fn hit_blocklist(manga: &Manga, blocklist: &[String]) -> bool {
	...
	if let Some(ref categories) = manga.tags {
		all_tags.extend(categories.iter().cloned());
	}

	all_tags.iter().any(|tag| blocklist.contains(tag))
}
