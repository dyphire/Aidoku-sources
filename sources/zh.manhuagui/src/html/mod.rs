use crate::{USER_AGENT, decoder::Decoder, helper::encode_uri};
use aidoku::{
	Chapter, Manga, MangaPageResult, MangaStatus, Page, PageContent, Result, SelectFilter,
...
fn extract_chapter_number(title: &str) -> Option<f32> {
	let re = Regex::new(r"(?:第\s*)(\d+(?:\.\d+)?)|(\d+(?:\.\d+)?)\s*(?:话|話|章|回|卷|册|冊)")
...
impl MangaPage for Document {
	fn manga_page_result(&self) -> Result<MangaPageResult> {
		...
		let elements = self
			.select(selector)
			.ok_or_else(|| aidoku::error!("Failed to select manga list"))?;
		for element in elements {
			let link_element = if selector == ".cf > .book-cover > a" {
				...
				element
					.select_first("a")
					.ok_or_else(|| aidoku::error!("Failed to select link"))?
			};

			let href = link_element
				.attr("href")
				.ok_or_else(|| aidoku::error!("Failed to get href"))?;
			let manga_id = href.replace("/comic/", "").replace('/', "");
			let title = link_element
				.attr("title")
				.ok_or_else(|| aidoku::error!("Failed to get title"))?;
			let manga = Manga {
				key: manga_id.clone(),
	...
	fn update_details(&self, manga: &mut Manga) -> Result<()> {
		...
		let title_element = self
			...
			.ok_or_else(|| aidoku::error!("Failed to select title"))?;
		...
		let desc_element = self
			...
			.ok_or_else(|| aidoku::error!("Failed to select description"))?;
		manga.description = Some(
			desc_element
				.text()
				.ok_or_else(|| aidoku::error!("Failed to get description"))?,
		);

		let status_element = self
			.select_first("li.status")
			.ok_or_else(|| aidoku::error!("Failed to select status"))?;
		let status_text = status_element
			.text()
			.ok_or_else(|| aidoku::error!("Failed to get status"))?;
...
impl ChapterPage for Document {
	fn chapters(&self) -> Result<Vec<Chapter>> {
		...
		if let Some(h4_elements) = div.select("h4") {
			for h4 in h4_elements {
				if let Some(span) = h4.select_first("span") {
					if let Some(scanlator) = span.text() {
		...
		let chapter_list_elements = div
			.select(".chapter-list")
			.ok_or_else(|| aidoku::error!("Failed to select chapter list"))?;
		for element in chapter_list_elements.rev() {
				...
				for ul_ref in ul_elements {
					...
					if let Some(li_elements) = ul.select("li") {
						for li_ref in li_elements.rev() {
							...
							let a_element = elem
								.select_first("a")
								.ok_or_else(|| aidoku::error!("Failed to select a"))?;
							let url = a_element
								.attr("href")
								.ok_or_else(|| aidoku::error!("Failed to get href"))?;
							...
							if let Some(i_element) = elem.select_first("i") {
								if let Some(page_text) = i_element.text() {
...
impl GenresPage for Document {
	fn filter(&self) -> Result<SelectFilter> {
		let genre_links = self
			...
			.ok_or_else(|| aidoku::error!("Failed to select genre filter links"))?;

		let (options, ids): (Vec<_>, Vec<_>) = genre_links
			.filter_map(|element| {
				let option = element.text()?;
				let href = element.attr("href")?;
				let id = href.trim_matches('/').split('/').last()?.into();
...
pub fn get_page_list(base_url: String) -> Result<Vec<Page>> {
	...
	for (_index, str) in pages_str.into_iter().enumerate() {
